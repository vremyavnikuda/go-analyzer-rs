...
impl Backend {
    fn new(client: Client) -> Self {
        ...
        parser.set_language(language()).expect("Failed to set Go language");
        Backend {
            client,
    ...
    async fn run_go_vet(&self, uri: &Url) -> Vec<(u32, String)> {
        ...
        let output = match
            std::process::Command::new("go").arg("vet").arg("-race").arg(path).output()
        {
            Ok(output) => output,
            Err(_) => {
                return vec![];
...
impl LanguageServer for Backend {
    ...
    async fn execute_command(
        ...
        if params.command == "goanalyzer/cursor" {
            self.client.log_message(MessageType::INFO, "Executing goanalyzer/cursor").await;
            self.client.send_notification::<ProgressNotification>(
                "Starting analysis...".to_string()
            ).await;
            let args: TextDocumentPositionParams = serde_json
                ::from_value(params.arguments[0].clone())
                .map_err(|e| {
            ...
            let race_warnings = self.run_go_vet(&uri).await;
            ...
            if let Some(mut var_info) = find_variable_at_position(&tree, code, position) {
                ...
                for use_range in var_info.uses.iter() {
                    decorations.push(Decoration {
                        range: *use_range,
                        kind: if var_info.is_pointer {
                            DecorationType::Pointer
                        } else {
                            DecorationType::Use
                        },
                        hover_text: format!("Use of `{}`", var_info.name),
                    });
                }
                if !race_warnings.is_empty() {
                    var_info.potential_race = true;
                    if let Some(last_use) = var_info.uses.last() {
                        let warning_text = race_warnings
                            .into_iter()
                            .map(|(_, msg)| msg)
                            .collect::<Vec<String>>()
                            .join("; ");
                        decorations.push(Decoration {
                            range: *last_use,
                            kind: DecorationType::Race,
                            hover_text: format!("Potential data race detected: {}", warning_text),
                        });
                    }
                }
                let value = serde_json
                    ::to_value(&decorations)
...
fn find_variable_at_position(tree: &Tree, code: &str, pos: Position) -> Option<VariableInfo> {
    ...
    fn traverse<'a>(
        ...
        eprintln!("Visiting node: kind={}, range={:?}", node.kind(), node_to_range(node));
                ...
                if let Some(child) = node.child(i) {
                    if child.kind() == "identifier" {
                        ...
                        if let Some(name) = code.get(byte_range) {
                            ...
                            // Если курсор находится на объявлении или мы уже нашли переменную
                            if
                                (child.start_position() <= point &&
                                    point <= child.end_position()) ||
                                var_info.as_ref().map_or(false, |v| v.name == name)
                            {
                                if var_info.is_none() {
        ...
        if let Some(ref mut info) = var_info {
            if node.kind() == "identifier" && node_to_range(node).start != info.declaration.start {
        ...
        if cursor.goto_first_child() {
            loop {
                traverse(cursor, code, pos, var_info);
    ...
    traverse(&mut cursor, code, pos, &mut var_info);
    var_info
}
