...
mod tests {
    ...
    // === Tests for Variable Finding at Cursor Position ===
...
func main() {
    ...
    println(x)
        ...
        let pos_decl = Position::new(2, 4); // Position on "x" in "x := 42"
        let var_info =
        ...
        assert!(var_info.uses.len() >= 1); // At least one usage in println(x)
...
func main() {
    ...
    println(user.name)
        ...
        let pos_object = Position::new(7, 4); // Position on "user" in "user.name"
        let var_info_obj =
        ...
        assert!(var_info_obj.uses.len() >= 2); // At least two uses in field accesses
    }
...
func process(data string) {
    ...
    for i := 0; i < 5; i++ {
        ...
        let pos_use = Position::new(4, 19); // Position on "data" in loop
        let var_info_use =
...
func main() {
    ...
    for i, v := range items {
        ...
        let pos_index = Position::new(3, 8); // Position on "i" in range clause
        ...
        assert!(var_info_i.uses.len() >= 1); // At least one use in println
    }
...
func main() {
    ...
    case string:
        ...
        let pos_switch = Position::new(3, 11); // Position on "v" in switch statement
        ...
        assert!(var_info.uses.len() >= 1); // Used in case branches
    }

    // === Tests for Variable Lifecycle Analysis ===
...
func demo() {
    ...
    println(x, y, z)    // Multiple uses
        ...
        // Analyze variable 'x'
        let pos_x = Position::new(2, 8); // Position on "x" in declaration
        let var_info =
            find_variable_at_position(&tree, code, pos_x).expect("Variable x should be found");

        assert_eq!(var_info.name, "x");
        assert!(var_info.declaration.start.line <= 2);
        assert!(var_info.uses.len() >= 2); // Used in &x, z := x + 5, println
    }
...
func main() {
    ...
    println(x, ptr)
        ...
        let pos_addr = Position::new(3, 12); // Position on "x" in "&x"
        let var_info =
...
func main() {
    ...
    println(user)
        ...
        let pos_var_decl = Position::new(2, 4); // Position on "user" in declaration
        let context =
...
func main() {
    ...
    // === Original existing tests maintained ===
...
func main() {
    ...
    println(value)
        ...
        assert!(counts.variables >= 2); // ch and value
        ...
        assert!(var_info.uses.len() >= 2); // Used in send and receive operations
    }
